...
class KfoldDataloader(pl.LightningDataModule):
    ...
    def setup(self, stage='fit'):
        if stage == 'fit':
            ...
            train_data = pd.read_csv(self.train_path)
            val_data = pd.read_csv(self.dev_path)
            total_data = pd.concat([train_data,val_data]).reset_index(drop=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nums_splits', default=3, type=int)
    ...
    for k in range(args.nums_splits):
        kfdataloader = KfoldDataloader(args.model_name, args.batch_size, args.shuffle, args.train_path,
                                    args.dev_path, args.test_path, args.predict_path, 
                                    k=k, split_seed=args.split_seed, num_splits=args.nums_splits)
    ...
    # 모델의 평균 성능
    score = sum(result) / args.nums_splits
