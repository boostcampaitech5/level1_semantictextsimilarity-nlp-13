...
class KfoldDataloader(pl.LightningDataModule):
    def __init__(self, model_name, batch_size, shuffle, train_path, dev_path, test_path, predict_path, k: int = 1, split_seed: int = 12345, num_splits: int = 10):
        super().__init__()
        self.model_name = model_name
        self.batch_size = batch_size
        self.shuffle = shuffle
    ...
    def setup(self, stage='fit'):
        if stage == 'fit':
            ...
            # 학습데이터와 검증데이터를 합쳐 전체 데이터에 대해 kfold를 수행합니다.
            total_inputs = train_inputs + val_inputs
            total_targets = train_targets + val_targets
            total_dataset = Dataset(total_inputs, total_targets)

            # 데이터셋 num_splits 번 fold
            kf = KFold(n_splits=self.num_splits,
                       shuffle=self.shuffle, random_state=self.split_seed)
