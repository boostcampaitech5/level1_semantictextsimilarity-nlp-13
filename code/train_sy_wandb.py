...
class Model(pl.LightningModule):
    def __init__(self, model_name, lr):
        ...
        # 사용할 모델을 호출합니다.
        self.plm = transformers.AutoModel.from_pretrained(pretrained_model_name_or_path=model_name,num_labels=1)
...
if __name__ == '__main__':
    ...
    dataloader = Dataloader(config['token_name'], config['train_setting']['batch_size'], config['train_setting']['shuffle'], config['dir']['train_path'], config['dir']['dev_path'],
    ...
    trainer = pl.Trainer(accelerator='gpu', max_epochs=config['train_setting']['max_epoch'], logger=wandb_logger, log_every_n_steps=1, default_root_dir=folder_name,
                     callbacks = [lr_monitor,early_stop_callback,checkpoint_callback_val,checkpoint_callback_pearson,checkpoint_callback_recent])
